...
PYTHON27_ONLY="cogent OBITools gdata qcli emperor preprocess Amara pysqlite IPTest ipaddress functools32 blmath bamsurgeon"
...
export TORCH_CUDA_ARCH_LIST="8.0;9.0+PTX" # for torch and others using it
export CUDAARCHS="80;90;90-virtual" # for cmake, used to set CMAKE_CUDA_ARCHITECTURES
...
function wrapped_pip_install {
	...
	DOWNLOADED_DEPS=$(grep Downloading $TMPFILE | sed -e 's/\.metadata//' -e 's/\.zip//' -e 's/\.tar\.gz//' | awk '{if(!seen[$2]++) print $2;}')
	if [[ ! -z "$DOWNLOADED_DEPS" && $RECURSIVE -eq 1 ]]; then
		...
		for w in $DOWNLOADED_DEPS; do
			...
			wheel_version=$(basename $w | sed -e "s/^${wheel_name}-//" | cut -d'-' -f1 | cut -d'+' -f1")
...
function verify_and_patch_arch_flags()
{
	...
	files_native=$(grep -rl --exclude-dir="doc*" --exclude='*.md' --exclude='README*' --exclude='INSTALL*' --exclude='*.tex' --exclude='NEWS' -- "-march=native" . )
	files_xHost=$(grep -rl  --exclude-dir="doc*" --exclude='*.md' --exclude='README*' --exclude='INSTALL*' --exclude='*.tex' --exclude='NEWS' -- "-xHost" .)

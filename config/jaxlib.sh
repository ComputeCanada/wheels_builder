...
if [[ $THIS_SCRIPT == 'build_wheel.sh' ]]; then
        ...
        bash unmanylinuxize.sh --package jaxlib --add_path $CUDA_HOME/lib64:$EBROOTCUDNN/lib --version ${VERSION:?A version is required} --find_links=https://storage.googleapis.com/jax-releases/jax_releases.html
        exit 1;
fi
